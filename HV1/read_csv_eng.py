...
with open('menntun_eng.csv', 'r') as csvfile:
    ...
    for row in dictreader:
        ...
        menntun_aldurbus.add(row['Aldursflokkur/Buseta'])
        menntun_kyn.add(row['Kyn'])

with open('laun_alm_vinnum_eng.csv', 'r') as csvfile:
    # Sniff
    currdialect = csv.Sniffer().sniff(csvfile.read(1024))
    # Go back
    csvfile.seek(0)
    ...
    for row in dictreader:
        vinna.append(row)
        vinna_ar.add(row['Ar'])
        vinna_starfsstett.add(row['Starfsstett'])
...
vinna_vinnuhlutf = [x for x in list(vinna[0].keys()) if x != 'Kyn' and x != 'Starfsstett' and x != 'Ar']
